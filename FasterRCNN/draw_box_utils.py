...
def draw_text(draw,
    ...
    display_str_heights = [font.font.getsize(ds)[1] for ds in display_str]
    ...
    for ds in display_str:
        text_width, text_height = font.font.getsize(ds)
...
def draw_arrow_start_end_node(image, arrows):
    draw = ImageDraw.Draw(image)
    for arrow in arrows:
        try:
            draw.rectangle([arrow['start'], arrow['end']],fill=ImageColor.getrgb("Red"))
        except:
            print(f"Error arrow: {arrow}")
        # draw.point(arrow['start'], fill=ImageColor.getrgb("Red"))
        # draw.point(arrow['end'], fill=ImageColor.getrgb("Red"))
    return image