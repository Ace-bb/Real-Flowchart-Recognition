...
def plot_loss_and_lr(train_loss, learning_rate, save_path = None):
    try:
        ...
        if save_path==None:
            ...
            fig.savefig(f'{save_path}/loss_and_lr{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}.png')
        plt.close()
...
def plot_map(mAP, save_path = None):
    try:
        ...
        if save_path==None:
            ...
            plt.savefig(f'{save_path}/mAP.png')
